...
AI 的人格由**提示词文件**决定，默认是空的 `default.txt`（即无特殊人设）。

| 文件 | 位置 | 作用 |
|------|------|------|
| `group_prompts/default.txt` | 配置目录下 | **群聊**时 AI 的系统提示词（人设） |
| `private_prompts/default.txt` | 配置目录下 | **私聊**时 AI 的系统提示词（人设） |

> 配置文件目录：`C:\Users\wangzunshi\AppData\Roaming\nonebot2\nonebot_plugin_suggarchat\`
> 修改 `.txt` 文件后插件会**自动热重载**（无需重启 bot）。

**示例**（写入 `group_prompts/default.txt`）：
```
...
| `/prompt --(set) <文字>` | 群管理 | **设置当前群/会话的提示词**（人设，≤1000 字） |
